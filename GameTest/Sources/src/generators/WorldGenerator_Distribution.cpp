...
bool WorldGenerator_Distribution::loadFromXML(const std::string &fileDirectory, TiXmlElement *element, WorldGenerator *worldGenerator)
{
    bool loaded = true;
    ...
    while(spawnPointChild != nullptr)
    {
        ...
        if(spawnPointElement)
        {
            ...
            m_spawnPoints.back().loadFromXML(fileDirectory, spawnPointElement, worldGenerator);
        }
                                                  ...
                                                  bool generateCharacters, pou::RNGenerator *rng)
{
    ...
    glm::ivec2 nbrSections = glm::ceil(m_worldGenerator->terrain()->getExtent()/m_parameters.sectionSize);
    ...
    for(float x = 0 ; x < nbrSections.x ; ++x)
    {
        for(size_t curPoint = 0 ; curPoint < distributedPoints.distributedPositions->size() ; ++curPoint)
        {
            ...
            auto pointWorldPos = pointPos; // + glm::vec2(x,y) * m_parameters.sectionSize - (m_worldGenerator->terrain()->getExtent()) * 0.5f;
            if(m_parameters.type == WorldGenerator_DistributionType_Uniform
            || m_parameters.type == WorldGenerator_DistributionType_Poisson
            || m_parameters.type == WorldGenerator_DistributionType_Path)
                pointWorldPos += glm::vec2(x,y) * m_parameters.sectionSize - (m_worldGenerator->terrain()->getExtent()) * 0.5f;

            if(m_parameters.useGridPosition)
                pointWorldPos = glm::round(pointWorldPos/m_worldGenerator->terrain()->getTileSize()) * m_worldGenerator->terrain()->getTileSize();

            if(m_worldGenerator->terrain()->getSpawnType(pointWorldPos) < m_parameters.spawnType)
                continue;
...
WorldGenerator_Distribution_DistributedPoints WorldGenerator_Distribution::generatePointsDistribution(pou::RNGenerator *rng)
{
    ...
    if(m_parameters.sectionSize.x == 0 || m_parameters.sectionSize.y == 0)
        m_parameters.sectionSize = m_worldGenerator->terrain()->getExtent();
    ...
    if(m_parameters.type == WorldGenerator_DistributionType_Uniform)
    {
        ...
        m_parameters.sectionSize = m_worldGenerator->terrain()->getTileSize() * m_parameters.distance;
    ...
    {
        m_otherSampler = m_worldGenerator->getSpawnGroup(m_parameters.spawnGroupName);
        distributedPoints.distributedPositions = &m_otherSampler;
    }
...
WorldGenerator_Distribution_DistributedPoints WorldGenerator_Distribution::generatePathDistribution(pou::RNGenerator *rng)
{
    ...
    auto rasterizedPaths = m_worldGenerator->terrain()->getRasterizedPaths(m_parameters.pathName);
    ...
    for(auto rasterizedPath : *rasterizedPaths)
    {
        ...
        {
            auto pointPos = glm::vec2(rasterizedPath[p])*m_worldGenerator->terrain()->getTileSize(); //m_terrain->gridToWorldPosition(rasterizedPath[p].x, rasterizedPath[p].y);
            m_otherSampler.push_back(pointPos);
